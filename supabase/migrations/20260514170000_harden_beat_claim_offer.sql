alter table public.beat_claims add column if not exists ip_address text;
alter table public.beat_claims add column if not exists user_agent text;
alter table public.beat_claims add column if not exists device_fingerprint text;

create index if not exists beat_claims_ip_address_idx on public.beat_claims (ip_address) where ip_address is not null;
create index if not exists beat_claims_device_fingerprint_idx on public.beat_claims (device_fingerprint) where device_fingerprint is not null;
...
begin
  ...
  where c.purchased_at is null
    and (
      ...
      or (ip_norm is not null and c.ip_address = ip_norm)
    )
