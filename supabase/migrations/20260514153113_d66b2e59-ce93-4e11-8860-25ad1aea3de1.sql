-- Ensure release_at exists on beats
...
drop policy if exists "Admins can read all beat claims" on public.beat_claims;
create policy "Admins can read all beat claims"
  on public.beat_claims for select
  using (public.has_role(auth.uid(), 'admin'::app_role));

drop policy if exists "Admins can update all beat claims" on public.beat_claims;
create policy "Admins can update all beat claims"
  on public.beat_claims for update
  using (public.has_role(auth.uid(), 'admin'::app_role))
  with check (public.has_role(auth.uid(), 'admin'::app_role));

drop policy if exists "Admins can delete beat claims" on public.beat_claims;
create policy "Admins can delete beat claims"
  on public.beat_claims for delete
  using (public.has_role(auth.uid(), 'admin'::app_role));
...
returns table (
  id uuid, title text, producer_name text, genre text, mood text,
  music_key text, bpm integer, duration_seconds integer,
  cover_url text, audio_url text, audio_url_tagged text, created_at timestamptz
)
language sql stable security definer set search_path = public as $$
  select b.id, b.title, b.producer_name, b.genre, b.mood, b.music_key,
         b.bpm, b.duration_seconds, b.cover_url, b.audio_url, b.audio_url_tagged, b.created_at
  from public.beats b
...
begin
  ...
  where lower(c.email) = email_norm and c.expires_at > now()
  order by c.created_at desc limit 1;

  if existing.id is not null then
    token := existing.token; expires_at := existing.expires_at;
    beat_id := existing.beat_id; reused := true;
    return next; return;
  end if;
  ...
  token := inserted.token; expires_at := inserted.expires_at;
  beat_id := inserted.beat_id; reused := false;
  return next;
end;
$$;
...
returns table (
  claim_id uuid, token text, email text, expires_at timestamptz,
  created_at timestamptz, purchased_at timestamptz,
  beat_id uuid, title text, producer_name text, genre text, mood text,
  music_key text, bpm integer, duration_seconds integer,
  cover_url text, audio_url text, audio_url_tagged text
)
language sql stable security definer set search_path = public as $$
  select c.id, c.token, c.email, c.expires_at, c.created_at, c.purchased_at,
         b.id, b.title, b.producer_name, b.genre, b.mood, b.music_key,
