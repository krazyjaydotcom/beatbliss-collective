...
function CheckoutPage() {
  const search = useSearch({ from: "/checkout" }) as { plan?: PlanId };
      ...
      <main className="container mx-auto px-6 py-10 max-w-2xl">
        ...
        {!submittedEmail ? (
          ...
          <div className="mt-8 rounded-2xl border border-border bg-card p-2 md:p-4">
            ...
            <CheckoutForm plan={plan} email={submittedEmail} />
          </div>
        )}
      </main>
    </div>
...
function CheckoutForm({ plan, email }: { plan: PlanId; email: string }) {
  ...
  const fetchClientSecret = async (): Promise<string> => {
    ...
    const result = await create({
      data: { priceId: plan, email, returnUrl, environment: getStripeEnvironment() },
    });
